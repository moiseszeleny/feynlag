...
def _render_rows(rows, sep=r"\;\text{multiplies}\;", fallback_sep="multiplies"):
    ...
    if _in_notebook():
        body = r"\\".join(
            rf"{sp.latex(a)} &{sep}& {sp.latex(b)}" for a, b in rows)
        _math(rf"\begin{{array}}{{rcl}}{body}\end{{array}}")
        return
...
def check_invariant(expr, transform, label, simplifier=sp.simplify):
    ...
    if not callable(transform):
        ...
        if not any(expr.has(k) for k in transform):
            raise ValueError(
                f"check_invariant: the expression contains none of "
                f"{sorted(map(str, transform))}, so the substitution is a no-op "
                f"and the check is vacuous.")
...
class Ledger:
    ...
    def to_latex(self, path, subtitle=""):
        ...
        """
        body = [_TEX_PREAMBLE, _tex_title(_tex_escape(self.title),
                                          _tex_escape(subtitle))]
        for i, s in enumerate(self.steps, 1):
            tag = {"derived": "derived here", "imported": "imported",
                   "assumed": "assumed"}[s.obtained]
            body.append(rf"\subsection*{{{i}.\quad {_tex_escape(s.claim)}}}")
            section = (rf" --- \S{_tex_escape(s.section).replace('-', '--')}"
                       if s.section else "")
            body.append(rf"\emph{{{tag}}}{section}\par\medskip")
            if s.statement is not None:
                body.append(r"\textbf{Claim}\par\nopagebreak")
                body.append(rf"\begin{{equation*}}\small "
                            rf"{_as_latex(s.statement)} \end{{equation*}}")
            if s.expr is not None:
                if s.statement is not None:
            ...
            if s.checks:
                ...
                body += [rf"\item {_tex_escape(c)}" for c in s.checks]
                body.append(r"\end{itemize}")
            if s.falsified_by:
                body.append(rf"\textbf{{Would be falsified by:}} "
                            rf"{_tex_escape(s.falsified_by)}\par")
...
#: Everything above the title.  Kept a plain string, not an f-string: it is all
#: literal LaTeX braces, which an f-string would make us double.  (The ``%`` here
#: are LaTeX comments — single, since nothing %-formats this any more.)
_TEX_PREAMBLE = r"""% Auto-generated by research/thdm_s3/derive.py -- do not edit by hand.
...
def _tex_title(title, subtitle):
    """The ``\\title``/``\\maketitle`` block — the only part with placeholders."""
    return (rf"\title{{Derivation appendix: {title}\\[4pt]"
            rf"\large\normalfont {subtitle}}}"
            "\n" r"\date{}" "\n" r"\begin{document}" "\n" r"\maketitle" "\n")
...
#: Font commands to strip in `_tex_to_console`, as an argument (``\mathrm{tr}``),
#: applied to one token (``\mathsf T``) or as a bare switch (``\rm SM``).  Built by
#: concatenation, not f-strings: the patterns are full of ``{}`` that an f-string
#: would make us double.  Compiled once here rather than per call.
_FONT = r"mathrm|mathsf|mathbb|mathcal|mathit|text|rm|bf|it|sf"
_RE_FONT_ARG = re.compile(r"\\(?:" + _FONT + r")\s*\{([^{}]*)\}")
_RE_FONT_TOKEN = re.compile(r"\\(?:" + _FONT + r")\s+(\w)")
_RE_FONT_BARE = re.compile(r"\\(?:" + _FONT + r")(?![A-Za-z])\s*")

#: Accents become combining marks, so ``\hat n`` reads as n̂ and not "hat n".
_ACCENT_RES = tuple(
    (re.compile(r"\\" + cmd + r"\s*\{([^{}]*)\}|\\" + cmd + r"\s*(\w)"), mark)
...
def _tex_to_console(s):
    ...
    def _one(m):
        ...
        body = _RE_FONT_ARG.sub(r"\1", body)
        body = _RE_FONT_TOKEN.sub(r"\1", body)
        body = _RE_FONT_BARE.sub("", body)
        for pattern, mark in _ACCENT_RES:
            body = pattern.sub(
...
def _tex_expr(expr):
    ...
    try:
        ...
        pass
    return rf"\begin{{equation*}}\small {sp.latex(expr)} \end{{equation*}}"
